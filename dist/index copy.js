...
!async function () {
    ...
    client.on("message", async function (message) {
        ...
        if (message.content.startsWith('=testgame')) {
            ...
            const teamsIndex = [...new Set(Object.values(resultsObject.players).map((p) => p.team))];
            const results = Object.values(resultsObject.players).sort((a, b) => teamsIndex.indexOf(b) - teamsIndex.indexOf(a));
            const colour = results.find((r) => r.wins > 0).team;
            const winningTeam = { colour, name: colourMap.get(colour) };
            const db = await database_1.default;
            const players = await utils_1.Players.getManyByDiscord(results.map((r) => r.discord));
            let game = await db.activeGame.findOne({ "botIGN": bot_1.default }) || { gameNumber: 0, _id: new mongodb_1.ObjectId(), botIGN: bot_1.default };
            const [_winstreaks, _bedstreaks, _ratings, _kills, _result] = results.reduce((a, b) => {
                const player = players.get(b.discord);
                a[0].push(player?.winstreak ?? 0);
                a[1].push(player?.bedstreak ?? 0);
                a[2].push(player?.elo ?? 0);
                a[3].push(b.kills);
                return a;
            }, [[], [], [], [], results[0].wins ?? 0]);
            const ratings = utils_1.HerrsELO(_ratings, _kills, _winstreaks, _bedstreaks, _result === 1 ? 1 : 0, 64).map((elo, index) => {
                if (elo <= 10 && index >= comparisonPoint && index < comparisonPoint + results.length / 2) {
                    while (elo <= 10) {
                        elo += Math.round((5000 - _ratings[index]) / 300);
                    }
                }
                else if (elo >= 0 && (index >= comparisonPoint + results.length / 2 || index < comparisonPoint)) {
                    while (elo >= 0) {
                        elo -= Math.round((5000 - _ratings[index]) / 300);
                    }
                }
            ...
            const teams = {};
            const comparisonPoint = _result ? 1 : 0;
            const guild = await bot_1.defaultGuild;
            const statistics = players.array().map((p, i) => {
                const player = resultsObject.players[p.minecraft.name];
                if (!player)
                    return null;
                const updated = {
                    ...p.data,
                    bedstreak: player.bedsBroken ? (p.bedstreak ?? 0) + 1 : 0,
                    winstreak: player.wins ? (p.winstreak ?? 0) + 1 : 0,
                    losestreak: player.losses ? (p.losestreak ?? 0) + 1 : 0,
                    kills: p.data.kills + player.kills,
                    deaths: p.data.deaths + player.deaths,
                    bedsLost: p.data.bedsLost + player.bedsLost,
                    bedsBroken: p.data.bedsBroken + player.bedsBroken,
                    games: p.data.games + 1
                };
                updated.elo = Math.max(0, p.data.elo + (player.bedsBroken || 0) * 10 + ratings[i]);
                guild.members.fetch(p.data.discord)
                    .then(p => p.setNickname(`[${updated.elo}] ${player.minecraft.name}`))
                    .catch(() => { });
                if (player.team) {
                    const entry = {
                        kills: player.kills,
                        deaths: player.deaths,
                        destroyedBed: (player.bedsLost ?? 0) > 0,
                        username: player.minecraft.name,
                        winstreak: p.data.winstreak,
                        bedstreak: p.data.bedstreak,
                        discord: p.data.discord,
                        oldRating: p.data.elo,
                        newRating: updated.elo
                    };
                    if (!teams[player.team])
                        teams[player.team] = { players: [entry], winner: (player.wins ?? 0) > 0 };
                    else
                        teams[player.team].players.push(entry);
                }
                return updated;
            }).filter(s => s !== null);
            ...
            await Promise.all([
                utils_1.gameReport(teams[teamColours[0]].players.map((p) => {
                    const role = getRole(p.newRating);
                    const old = getRole(p.oldRating);
                    if (role !== old) {
                    ...
                    return `**${p.username}** | \`[${p.oldRating} → ${p.newRating}]\` ${role !== old ? role : ''}`;
                }), teams[teamColours[1]].players.map((p) => {
                    const role = getRole(p.newRating);
                    const old = getRole(p.oldRating);
                    if (role !== old) {
                        guild.members.fetch(p.discord).then(m => {
                    ...
                    return `**${p.username}** | \`[${p.oldRating} → ${p.newRating}]\` ${role !== old ? role : ''}`;
                }), game.gameNumber, `Team ${winningTeam}`, results.map((r) => `<@${r.discord}>`).join('')),
                _operation.execute(),
                db.games.updateOne({
                    _id: game._id
                }, {
            ...
            logger.info(`Successfully finished game ${game._id} (managed by ${bot_1.default}).`);
        }
